...
import { listAllReviews } from '@/lib/db/queries/admin'
import { z } from 'zod'
...
const search = z.object({ q: z.string().trim().min(1).max(100).optional().catch(undefined) })
...
export default async function Page({ searchParams }: PageProps<'/admin/reviews'>) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Suspense key={filters.q ?? ''} fallback={<Skeleton className="h-64 rounded-xl" />}>
        <Rows q={filters.q} />
      </Suspense>
    </div>
  )
...
async function Rows({ q }: { q?: string }) {
  const reviews = await listAllReviews(q)

  return (
    <IndexTable columns={['Review', 'Product', 'Rating', 'Left', '']} empty="No reviews match that.">
      {reviews.map((review) => (
        <tr key={`${review.user_id}-${review.product_id}`} className="hover:bg-olive-950/[0.03] dark:hover:bg-white/[0.03]">
